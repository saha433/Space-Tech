    ...
    def run(self, schedule: Dict[str, Any]) -> Telemetry:
        ...
        for az in azs:
            ...
            rw = rwFactory.create(
                "Honeywell_HR16",
                g_B,
                maxMomentum=50.0,  # Basilisk 2.10 HR16 accepts only 50/75/100 Nms presets.
                Omega=0.0,
            )
            # Match the hackathon spacecraft: 30 mNms max momentum at the
            # preset wheel speed.  Newer Basilisk rejects custom HR16
            # maxMomentum values, so override the derived rotor inertia here.
            rw.Js = self.cfg.sc_params["wheel_Hmax_Nms"] / rw.Omega_max
            rw.Jt = 0.5 * rw.Js
            rw.Jg = rw.Jt
            rw_objs.append(rw)
        ...
        # Modern Basilisk spacecraft publishes state; simpleNav converts it to
        # the attitude navigation message expected by attTrackingError.
        ...
        attErrorConfig.ModelTag = "attErr"
        sim.AddModelToTask(task_name, attErrorConfig)

        # MRP feedback control
        ctrlConfig = mrpFeedback.mrpFeedback()
        ctrlConfig.ModelTag = "mrpCtl"
        ctrlConfig.K   = 0.15
        ctrlConfig.Ki  = -1.0   # integral off; harness keeps the plant clean
        ctrlConfig.P   = 0.6
        ctrlConfig.integralLimit = 2.0 / ctrlConfig.P * 0.1
        sim.AddModelToTask(task_name, ctrlConfig)

        # Map body torque to wheel torques
        rwMotorConfig = rwMotorTorque.rwMotorTorque()
        rwMotorConfig.ModelTag = "rwMotor"
        rwMotorConfig.controlAxes_B = [1, 0, 0,
                                        0, 1, 0,
                                        0, 0, 1]
        sim.AddModelToTask(task_name, rwMotorConfig)

        # ---------- Wire messages ----------
        # BSK-VERIFY: message wiring API is `msgInName = msgOutName.addSubscriber()`
        # as of Basilisk >= 2.0.
        attErrorConfig.attNavInMsg.subscribeTo(nav.attOutMsg)
        ...
        veh_config_msg = _make_veh_config_msg(
            messaging, self.cfg.sc_params["inertia_kgm2"])
        ctrlConfig.vehConfigInMsg.subscribeTo(veh_config_msg)
        ...
        rw_avail = messaging.RWAvailabilityMsgPayload()
        rw_avail.wheelAvailability = [rwMotorTorque.AVAILABLE] * len(rw_objs)
        rw_avail_msg = messaging.RWAvailabilityMsg().write(rw_avail)

        rwMotorConfig.rwParamsInMsg.subscribeTo(rwFactory.getConfigMessage())
        rwMotorConfig.rwAvailInMsg.subscribeTo(rw_avail_msg)
